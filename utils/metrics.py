...
class DiceAverage(object):
    ...
    def update(self, logits, targets):
        ...
        self.sum += self.value
        self.count += 1
        self.avg = np.around(self.sum / self.count, 4) #around是指将计算出来的平均dice系数四舍五入到小数点后四位
        # print(self.value)
    ...
    def get_dices(logits, targets):
        ...
        for class_index in range(targets.size()[1]):
            inter = torch.sum(logits[:, class_index, :, :, :] * targets[:, class_index, :, :, :])# 计算交集，即模型预测为正类别并且实际也为正类别的像素数。
            union = torch.sum(logits[:, class_index, :, :, :]) + torch.sum(targets[:, class_index, :, :, :])# 计算并集，即模型预测为正类别或实际为正类别的像素数。
