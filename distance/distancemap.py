...
from distance.distances import manhattan_distance, euclidean_distance
from distance.functions import identity, square, linear
...
def distance_map_3d(map_shape, input_points, distance, omega, alpha):
    dm = np.full(map_shape, omega)#创建一个形状为map_shape的三维数组，并用omega进行填充，作为初始的距离图

    for p in input_points:#对于输入点p进行处理
        #初始化更新区域的边界变量
        x_min = 0
        x_max = map_shape[0]
        y_min = 0
