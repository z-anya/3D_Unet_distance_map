...
class Img_DataSet(Dataset):
    ...
    def recompone_result(self):#将哥哥预测的小块重新组合成一个完整的预测图象
    ...
    def padding_img(self, img, size, stride):# 对三维图像进行padding，确保图像尺寸适合后续的分块或者卷积
        assert (len(img.shape) == 3)  # 3D array
        img_s, img_h, img_w = img.shape
        leftover_s = (img_s - size) % stride# 计算深度方向上需要填充的切片数，
