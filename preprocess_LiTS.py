...
class LITS_preprocess:
    ...
    def fix_data(self):
        ...
        for ct_file,i in zip(file_list,range(Numbers)):
            ...
            seg_path = os.path.join(self.raw_root_path, 'mask', ct_file.replace('volume', 'segmentation'))
            ...
            if new_ct != None and new_seg != None:
                ...
                sitk.WriteImage(new_seg, os.path.join(self.fixed_path, 'mask', ct_file.replace('volume', 'segmentation').replace('.nii', '.nii.gz')))
    ...
    def process(self, ct_path, seg_path, classes=None):
        ...
        # # 计算当前图像z轴的slice数量
        # current_slices = ct_array.shape[0]
        # # 计算需要进行padding的slice数量
        # padding_slices = 320 - current_slices
        #
        # # 在z轴上进行padding，将padding值设置为0
        # ct_array = np.pad(ct_array, ((padding_slices - padding_slices // 2, padding_slices // 2), (0, 0), (0, 0)),
        #                   mode='constant', constant_values=0)
        # seg_array = np.pad(seg_array, ((padding_slices - padding_slices // 2, padding_slices // 2), (0, 0), (0, 0)),
        #                    mode='constant', constant_values=0)
        #
        # print("Padded shape:", ct_array.shape, seg_array.shape)
        # 保存为对应的格式
    ...
    def write_name_list(self, name_list, file_name):
        ...
        for name in name_list:
            ...
            seg_path = os.path.join(self.fixed_path, 'mask', name.replace('volume', 'segmentation'))
            f.write(ct_path + ' ' + seg_path + "\n")
        f.close()

if __name__ == '__main__':
    raw_dataset_path = '/opt/data/private/3DUNet-Pytorch-master2/train_val_data/train'
